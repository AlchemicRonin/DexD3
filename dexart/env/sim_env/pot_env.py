...
class PotEnv(BaseSimulationEnv):
    def __init__(
        ...
        if not self.change_instance_when_reset:
            ...
            self.l_handle = self.prismatic_joint.get_child_link()
            self.r_handle = self.prismatic_joint.get_parent_link()
    ...
    def update_handle_relative_pose(self, handle_name=None):
        ...
        if handle_name == "r_handle":
            for collision_mesh in self.r_handle.get_collision_shapes():
                ...
                for vertex in vertices:
                    ...
                    vertices_global_pose_list.append(
                        self.r_handle.get_pose().transform(vertex_relative_pose)
                    )
            ...
            relative_pose = self.r_handle.get_pose().inv().transform(handle_global_pose)
    ...
    def get_handle_global_pose(self):
        ...
        l_better_global_pose = self.r_handle.get_pose().transform(
