...
class BaseBimanualRLEnv(BaseRLEnv):
    ...
    def setup(self, robot_name):
        ...
        self.robot_l.set_pose(sapien.Pose(np.array([0.7, -0.3, 0])))
        self.robot.set_pose(sapien.Pose(np.array([0.7, 0.3, 0])))
    ...
    def split_action(self, action: np.ndarray):
        ...
        action, action_l = action[:self.arm_dof], action[self.arm_dof:]
        assert action.shape[0] == self.arm_dof and action_l.shape[0] == self.arm_dof_l
        return action, action_l
